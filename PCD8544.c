...
static void my_setpixel(pcdstruct_ptr pcd ,uint8_t x, uint8_t y, uint8_t color)
{
	...
	// x is which column
	if (color)
		pcd->pcd8544_buffer[x+ (y/8)*LCDWIDTH] |= _BV(y%8);
	else
		pcd->pcd8544_buffer[x+ (y/8)*LCDWIDTH] &= ~_BV(y%8);
}
...
void LCDshowLogo(pcdstruct_ptr pcd)
{
	...
	{
		pcd->pcd8544_buffer[i] = pi_logo[i];
	}
	LCDdisplay(pcd);
}
...
void LCDdrawstring(pcdstruct_ptr pcd, uint8_t x, uint8_t y, char *c)
{
	pcd->cursor_x = x;
	pcd->cursor_y = y;
	while (*c)
...
void LCDdrawstring_P(pcdstruct_ptr pcd, uint8_t x, uint8_t y, const char *str)
{
	pcd->cursor_x = x;
	pcd->cursor_y = y;
	while (1)
...
void LCDdrawchar(pcdstruct_ptr pcd, uint8_t x, uint8_t y, char c)
{
...
void LCDwrite(pcdstruct_ptr pcd, uint8_t c)
{
	...
	{
		LCDdrawchar(pcd, pcd->cursor_x, pcd->cursor_y, c);
...
void LCDdrawline(pcdstruct_ptr pcd,uint8_t x0, uint8_t y0, uint8_t x1, uint8_t y1, uint8_t color)
{
	...
	for (; x0<=x1; x0++)
	{
		if (steep)
		{
			my_setpixel(pcd, y0, x0, color);
		}
		else
		{
			my_setpixel(pcd, x0, y0, color);
		}
		err -= dy;
		if (err < 0)
		{
...
void LCDdrawrect(pcdstruct_ptr pcd, uint8_t x, uint8_t y, uint8_t w, uint8_t h, uint8_t color)
{
	...
	uint8_t i;
	for ( i=x; i<x+w; i++) {
		my_setpixel(pcd, i, y, color);
		my_setpixel(pcd, i, y+h-1, color);
	}
	for ( i=y; i<y+h; i++) {
		my_setpixel(pcd, x, i, color);
		my_setpixel(pcd, x+w-1, i, color);
	}
...
void LCDdrawcircle(pcdstruct_ptr pcd, uint8_t x0, uint8_t y0, uint8_t r, uint8_t color)
{
	...
	my_setpixel(pcd, x0, y0+r, color);
	my_setpixel(pcd, x0, y0-r, color);
	my_setpixel(pcd, x0+r, y0, color);
	my_setpixel(pcd, x0-r, y0, color);
	...
	while (x<y)
	{
		...
		my_setpixel(pcd, x0 + x, y0 + y, color);
		my_setpixel(pcd, x0 - x, y0 + y, color);
		my_setpixel(pcd, x0 + x, y0 - y, color);
		my_setpixel(pcd, x0 - x, y0 - y, color);

		my_setpixel(pcd, x0 + y, y0 + x, color);
		my_setpixel(pcd, x0 - y, y0 + x, color);
		my_setpixel(pcd, x0 + y, y0 - x, color);
		my_setpixel(pcd, x0 - y, y0 - x, color);
...
void LCDfillcircle(pcdstruct_ptr pcd, uint8_t x0, uint8_t y0, uint8_t r, uint8_t color)
{
	...
	while (x<y)
	{
		...
		for ( i=y0-y; i<=y0+y; i++)
		{
			my_setpixel(pcd, x0+x, i, color);
			my_setpixel(pcd, x0-x, i, color);
		}
		for ( i=y0-x; i<=y0+x; i++)
		{
			my_setpixel(pcd, x0+y, i, color);
			my_setpixel(pcd, x0-y, i, color);
		}
	}
}
...
uint8_t LCDgetPixel(pcdstruct_ptr pcd, uint8_t x, uint8_t y)
{
	...
	return (pcd->pcd8544_buffer[x+ (y/8)*LCDWIDTH] >> (7-(y%8))) & 0x1;
}
...
void LCDsetContrast(pcdstruct_ptr pcd, uint8_t val)
{
	...
	LCDcommand(pcd, PCD8544_FUNCTIONSET | PCD8544_EXTENDEDINSTRUCTION );
	LCDcommand(pcd, PCD8544_SETVOP | val);
	LCDcommand(pcd, PCD8544_FUNCTIONSET);
}
...
void LCDdisplay(pcdstruct_ptr pcd)
{
	...
	for(p = 0; p < 6; p++)
	{
		...
		LCDcommand(pcd, PCD8544_SETXADDR | col);
	...
	LCDcommand(pcd, PCD8544_SETYADDR );  // no idea why this is necessary but it is to finish the last byte?
#ifdef enablePartialUpdate
